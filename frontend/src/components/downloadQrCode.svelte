<script lang="ts">
  ...
  async function saveJson() {
    ...
    const dataToSave = {
        ...
        extension : selectedType,
        canvas : qrCode._canvas,
    };

    try {
        const response = await axios.post('http://localhost:3000/', dataToSave);
    ...
    <div class="flex-wrap">
      <button on:click={saveJson} class="p-1 font-bold text-white bg-blue-500 rounded hover:bg-blue-700">JSON</button>
      <button on:click={navigate}>Aller à l'autre page</button>
